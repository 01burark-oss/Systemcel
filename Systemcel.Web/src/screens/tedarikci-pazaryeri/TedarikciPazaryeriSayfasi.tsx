...
const statusLabels: Record<string, string> = { OdemeBekliyor: "Ödeme bekliyor", SiparisVerildi: "Sipariş verildi", Odendi: "Ödendi", TedarikciOnayladi: "Tedarikçi onayladı", Hazirlaniyor: "Hazırlanıyor", KismenSevkEdildi: "Kısmen sevk edildi", SevkEdildi: "Sevk edildi", KismenKabul: "Kısmen teslim alındı", TeslimEdildi: "Teslim edildi", HakEdisBekliyor: "Hakediş bekliyor", CariOdemeBekliyor: "Cari ödeme bekliyor", Tamamlandi: "Tamamlandı", IptalEdildi: "İptal edildi", KismiIptal: "Kısmen iptal edildi", IadeBekliyor: "İade bekliyor", IadeEdildi: "İade edildi", KismiIade: "Kısmen iade edildi", Itirazli: "İtirazlı" };
...
export function TedarikciPazaryeriSayfasi() {
  ...
  const [shippingForm, setShippingForm] = React.useState({ tasimaTipi: "Tedarikçi aracı", tasiyici: "", belgeNo: "", aracPlaka: "", surucuAdi: "", cikisDeposu: "", planlananTeslimAt: "", not: "" });
  const [shipmentLines, setShipmentLines] = React.useState<Record<number, { miktar: string; etiketSayisi: string; lotNo: string }>>({});
  ...
  const [receiptForm, setReceiptForm] = React.useState({ kabulEdilenMiktar: "", reddedilenMiktar: "0", redNedeni: "", not: "" });
  ...
  function openShipment(order: TedarikciSiparis) {
    ...
    setSelectedOrder(order);
    setShipmentLines(Object.fromEntries(lines.map((line) => [line.id, { miktar: String(line.miktar - (line.sevkEdilenMiktar ?? 0)), etiketSayisi: "1", lotNo: "" }])));
    setShippingForm({ tasimaTipi: "Tedarikçi aracı", tasiyici: "", belgeNo: "", aracPlaka: "", surucuAdi: "", cikisDeposu: "", planlananTeslimAt: "", not: "" });
    setModal("kargo");
  }

  async function createShipment() {
    if (!selectedOrder) return;
    const kalemler = Object.entries(shipmentLines).map(([id, line]) => ({ siparisKalemiId: Number(id), miktar: Number(line.miktar), etiketSayisi: Number(line.etiketSayisi), lotNo: line.lotNo, sonKullanmaTarihi: null, sicaklikMin: null, sicaklikMax: null }));
    try {
      const result = await send<{ etiketler: QrEtiketi[] }>(`/api/ekran/tedarikci-pazaryeri/tedarikci-siparisler/${selectedOrder.id}/sevkiyatlar`, "POST", { ...shippingForm, planlananTeslimAt: shippingForm.planlananTeslimAt || null, kalemler });
  ...
  async function resolveQr(value = qrCode) {
    ...
    try {
      ...
      setQrCode(result.kod); setQrResolution(result); setReceiptForm({ kabulEdilenMiktar: String(result.miktar), reddedilenMiktar: "0", redNedeni: "", not: "" }); setError("");
  ...
  return <main className="supplier-marketplace" aria-busy={busy}>
    ...
    {tab === "orders" ? <section className="supplier-marketplace__orders">
      <div className="supplier-marketplace__section-head"><div><h2>Siparişlerim</h2><p>Ürünü QR etiketiyle teslim al; kabul edilen miktar stok ve cariye işlensin.</p></div><button onClick={() => { setQrCode(""); setQrResolution(null); setModal("qrKabul"); }}><ScanLine />QR ile teslim al</button></div>
      {(data?.anaSiparisler ?? []).map((master) => {
        ...
        return <article key={master.id} className="supplier-marketplace__order-group"><header><div><small>{new Date(master.createdAt).toLocaleDateString("tr-TR")}</small><h2>{master.siparisNo}</h2><span>{statusLabels[master.durum] ?? master.durum}</span></div><strong>{money(master.genelToplam, master.paraBirimi)}</strong></header>{childOrders.map((order) => {
          ...
          return <OrderRow key={order.id} order={order} lines={data?.siparisKalemleri.filter((line) => line.tedarikciSiparisId === order.id) ?? []} actions={<>
            {["KismenSevkEdildi", "SevkEdildi", "KismenKabul"].includes(order.durum) ? <button onClick={() => { setQrCode(""); setQrResolution(null); setModal("qrKabul"); }}><QrCode />QR okut</button> : null}
    ...
    {tab === "sales" ? <section className="supplier-marketplace__orders"><div className="supplier-marketplace__section-head"><div><h2>Satışlarım</h2><p>Sipariş, sevkiyat, QR etiketi ve hakedişler</p></div><button onClick={() => { setEditingProductId(null); setProductForm(emptyProduct); setModal("urun"); }}><PackagePlus />Ürün ekle</button></div>{!data?.profil?.dogrulandi ? <p className="supplier-marketplace__notice">Ürün yayınlamak için tedarikçi başvurunuzu tamamlayın.</p> : null}{(data?.siparisler ?? []).filter((order) => order.tedarikciIsletmeId === data?.aktifIsletmeId).map((order) => {
      ...
      if (["Hazirlaniyor", "KismenSevkEdildi", "KismenKabul"].includes(order.durum)) actions.push(<button key="shipment" onClick={() => openShipment(order)}><QrCode />Sevkiyat ve QR oluştur</button>);
      ...
      return <OrderRow key={order.id} order={order} lines={data?.siparisKalemleri.filter((line) => line.tedarikciSiparisId === order.id) ?? []} actions={actions} showSettlement />;
...
function OrderRow({ order, lines, actions, showSettlement = false }: { order: TedarikciSiparis; lines: SiparisKalemi[]; actions: React.ReactNode; showSettlement?: boolean }) {
  return <section className="supplier-marketplace__order-row"><div className="supplier-marketplace__order-title"><Truck /><div><strong>{order.tedarikciUnvani}</strong><span>{order.siparisNo} · {statusLabels[order.durum] ?? order.durum}</span></div></div><ul>{lines.map((line) => <li key={line.id}><span>{line.ad} · {line.miktar} {line.birim}{line.sevkEdilenMiktar || line.kabulEdilenMiktar || line.reddedilenMiktar ? <small>Sevk {line.sevkEdilenMiktar ?? 0} · Kabul {line.kabulEdilenMiktar ?? 0}{line.reddedilenMiktar ? ` · Ret ${line.reddedilenMiktar}` : ""}</small> : null}</span><b>{money(line.toplamTutar, order.paraBirimi)}</b></li>)}</ul>{showSettlement ? <div className="supplier-marketplace__settlement"><span>Satış {money(order.genelToplam, order.paraBirimi)}</span><span>Kesintiler {money(order.komisyonTutari + order.komisyonKdvTutari + order.tevkifatTutari + order.odemeHizmetiBedeli, order.paraBirimi)}</span><strong>Hakediş {money(order.tedarikciHakEdisi, order.paraBirimi)}</strong></div> : null}{order.kargoTakipNo ? <p>{order.kargoFirmasi} · {order.kargoTakipNo}</p> : null}{actions ? <div className="supplier-marketplace__order-actions">{actions}</div> : null}</section>;
}
