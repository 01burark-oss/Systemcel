...
export function OdemeIncelemeSayfasi() {
  ...
  return <main className="admin-page payment-ops">
    <nav className="admin-subnav" aria-label="Yönetim bölümleri"><a href="/app/yonetim/muhasebeci-basvurulari">Muhasebeci başvuruları</a><a className="active" href="/app/yonetim/odemeler">Ödeme inceleme</a><a href="/app/yonetim/bildirim-teslimleri">Bildirim teslimleri</a><a href="/app/yonetim/muhasebeci-aktarimlari">Muhasebeci aktarımları</a><a href="/app/yonetim/destek">Destek talepleri</a></nav>
<section className="admin-page__toolbar"><div className="admin-page__stats" aria-label="Ödeme özeti"><Stat label="Toplam" value={data?.toplamSayisi ?? 0}/><Stat label="Başarılı" value={data?.basariliSayisi ?? 0}/><Stat label="Hatalı" value={data?.hataSayisi ?? 0}/><Stat label="İşlenemeyen olay" value={data?.islenemeyenOlaySayisi ?? 0}/></div><div className="admin-page__actions"><label className="admin-error-filter"><input type="checkbox" role="switch" className="app-switch" checked={sadeceHatalar} onChange={(event) => setSadeceHatalar(event.target.checked)}/>Yalnız hatalar</label><button type="button" onClick={() => yukle()} disabled={yukleniyor} aria-label="Yenile">{yukleniyor ? <Loader2 size={16} className="spin"/> : <RefreshCw size={16}/>}</button></div></section>
